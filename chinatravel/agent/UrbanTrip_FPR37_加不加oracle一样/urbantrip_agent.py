...
class UrbanTrip(BaseAgent):
    def __init__(self, **kwargs):
        ...
        # LLM 支持
        self.use_llm = kwargs.get("use_llm", True)  # 是否启用 LLM 辅助
        self.llm_cache = {}  # LLM 调用缓存

    def translate_nl2sl(self, query, load_cache=False):
        """
        Translate natural language query to symbolic language using LLM.
        This method is called when oracle_translation=False.
        """
        ...
        if load_cache and os.path.exists(file_path):
            print("[NL2SL] Loading translation from cache")
            query = load_json_file(file_path)
        else:
            print("[NL2SL] Calling LLM to translate NL to SL")
            query = nl2sl_reflect(query, self.backbone_llm)
            if "error" in query:
                query["hard_logic_py"] = {}
            save_json_file(query, file_path)
            print(f"[NL2SL] Translation completed and saved to cache")
    ...
    def run(self, query, prob_idx, oralce_translation=True, load_cache=True):
        ...
        # natural language -> symbolic language -> plan
        # If oracle_translation=True, use pre-annotated hard_logic from dataset
        # If oracle_translation=False, call LLM to translate NL to SL
        if not oralce_translation:
            print("[Oracle Translation] Disabled - Will use LLM to translate NL to SL")
            query = self.translate_nl2sl(query, load_cache=load_cache)
        else:
            print("[Oracle Translation] Enabled - Using pre-annotated hard_logic from dataset")
        ...
        # Add additional statistics to output
        total_time = time.time() - start_time
        plan_out["total_time_sec"] = total_time
        ...
        plan_out["total_token_count"] = self.backbone_llm.input_token_count + self.backbone_llm.output_token_count
        ...
        if oralce_translation:
            ...
            plan_out["nl2sl_source"] = "llm_generated"
            plan_out["nl2sl_reflect_count"] = query.get("reflect_cnt", 0)

        # Add constraint validation statistics
    ...
    def llm_rank_pois(self, poi_list, query_context, poi_type="attraction", top_k=None):
        """使用 LLM 对 POI 进行智能排序

        Args:
            poi_list: POI 候选列表
            query_context: 查询上下文（用户需求、当前位置等）
            poi_type: POI 类型（attraction/restaurant/accommodation）
            top_k: 返回前 k 个，None 表示返回所有

        Returns:
            排序后的 POI 列表
        """
        if not self.use_llm or not poi_list or len(poi_list) <= 1:
            return poi_list if top_k is None else poi_list[:top_k]

        try:
            # 构建 LLM prompt
            poi_infos = []
            for idx, poi in enumerate(poi_list[:20]):  # 最多考虑前 20 个候选
                info = f"{idx+1}. {poi.get('name', 'N/A')}"
                if 'rating' in poi:
                    info += f" (评分: {poi['rating']})"
                if 'price' in poi:
                    info += f" (价格: {poi['price']}元)"
                if 'address' in poi:
                    info += f" (位置: {poi['address']})"
                poi_infos.append(info)

            prompt = f"""你是一个旅行规划专家。请根据用户需求对以下{poi_type}进行排序推荐。

用户需求: {query_context.get('user_requirements', '无特殊要求')}
当前位置: {query_context.get('current_position', '未知')}
预算限制: {query_context.get('budget', '无限制')}

候选{poi_type}列表:
{chr(10).join(poi_infos)}

优化目标:
1. 满足用户偏好和约束
2. 地理位置合理（减少交通时间）
3. 性价比高（评分高、价格合理）
4. 多样性（不同类型的{poi_type}）

请直接返回排序后的编号列表，用逗号分隔，例如: 3,1,5,2,4
不要包含其他解释文字，只返回数字序列。"""
            ...
            if cache_key in self.llm_cache:
                ...
                # 调用 LLM（使用 __call__ 方法）
                response = self.backbone_llm([{"role": "user", "content": prompt}], one_line=True)

                # 累加 LLM 推理时间
                self.llm_inference_time_count += time.time() - llm_start_time
            ...
            ranking = [int(n) - 1 for n in numbers if 0 < int(n) <= len(poi_list)]

            # 重新排序 POI 列表
            ranked_pois = []
            used_indices = set()
            for idx in ranking:
                if idx < len(poi_list) and idx not in used_indices:
                    ranked_pois.append(poi_list[idx])
            ...
            result = ranked_pois if top_k is None else ranked_pois[:top_k]
            print(f"[LLM POI Ranking] Original: {len(poi_list)}, Ranked: {len(result)}, Top-K: {top_k}")
            return result

        except Exception as e:
            # Check if this is an InsufficientBalanceError by class name
            # (we can't use isinstance because there are multiple copies of this class)
            if type(e).__name__ == 'InsufficientBalanceError':
                print(f"\n⚠️  Authentication/Balance error detected in LLM POI ranking.")
                print(f"⚠️  Stopping execution immediately (no fallback allowed).\n")
                raise

            # For other errors, fallback to original order
            print(f"[LLM Ranking Error] {e}, falling back to original order")
            return poi_list if top_k is None else poi_list[:top_k]

    def llm_optimize_preferences(self, current_plan, query):
        """使用 LLM 优化偏好指标

        Args:
            current_plan: 当前生成的计划
            query: 用户查询
        ...
        if not self.use_llm:
            return {
                'target_attractions_per_day': 4,
                'target_restaurants_per_day': 3,
                'prefer_short_distance': True
            }

        try:
            prompt = f"""分析以下旅行计划的偏好优化策略。

目标城市: {query.get('target_city', 'N/A')}
旅行天数: {query.get('stay_days', 'N/A')}
人数: {query.get('people_number', 1)}

关键优化指标:
1. DAV (每天景点数): 目标 4 个/天，当前计划景点数/天数
2. DDR (每天餐饮数): 目标 3 个/天 (早中晚)
3. ATT (交通时间): 目标 15-30 分钟/次，选择地理位置近的 POI
...
{{
    ...
    "time_allocation": "合理分配时间"
            ...
            # (we can't use isinstance because there are multiple copies of this class)
            if type(e).__name__ == 'InsufficientBalanceError':
                print(f"\n⚠️  Authentication/Balance error detected in LLM optimization.")
                print(f"⚠️  Stopping execution immediately (no fallback allowed).\n")
                raise
        ...
        while len(candidates_type) > 0:
            ...
            # 如果是午餐、晚餐或景点
                ...
                if poi_type in ["lunch", "dinner"]:
                    ...
                    # === LLM 增强：智能排序候选餐厅 ===
                    if self.use_llm and len(candidate_attr_ranked) > 0:
                        # 构建查询上下文
                        query_context = {
                            'user_requirements': self.query.get('nature_language', ''),
                            'current_position': current_position,
                            'budget': self.restaurant_budget if self.restaurant_budget else '无限制',
                            'current_time': current_time,
                            'meal_type': poi_type
                        }
                        # 转换为字典列表供 LLM 排序
                        candidate_dicts = candidate_attr_ranked.to_dict('records')
                        # 使用 LLM 排序
                        ranked_dicts = self.llm_rank_pois(
                            candidate_dicts,
                            query_context,
                            poi_type="restaurant",
                            top_k=None
                        ...
                        # 转回 DataFrame
                        if ranked_dicts and len(ranked_dicts) > 0:
                            candidate_attr_ranked = pd.DataFrame(ranked_dicts)
                            print(f"[LLM] Reranked {len(ranked_dicts)} restaurants")
                ...
                # 如果是景点
                    ...
                    if self.use_llm and len(candidate_attr_ranked) > 0:
                        ...
                        query_context = {
                            'user_requirements': self.query.get('nature_language', ''),
                            'current_position': current_position,
                            'budget': self.attraction_budget if self.attraction_budget else '无限制',
                            'current_time': current_time,
                            'current_day': current_day,
                            'visited_attractions': list(self.attraction_names_visiting)
                        }
                        # 转换为字典列表供 LLM 排序
                        candidate_dicts = candidate_attr_ranked.to_dict('records')
                        # 使用 LLM 排序
                        ...
                        if ranked_dicts and len(ranked_dicts) > 0:
                            candidate_attr_ranked = pd.DataFrame(ranked_dicts)
                            print(f"[LLM] Reranked {len(ranked_dicts)} attractions")
    ...
    def extract_user_constraints_by_DSL(self, query):
        ...
        # 禁用 hard_logic_py 读取，一视同仁处理所有数据集
        # 统一转成字符串，防止 None 或非字符串类型
        dsl = ""  # 强制为空，不读取 hard_logic_py
        # dsl = query.get("hard_logic_py", "")  # 已禁用
    ...
    def constraints_validation(self, query, plan, poi_plan):
        ...
        # 禁用 hard_logic_py 验证，一视同仁处理所有数据集
        # 所有数据集统一不使用 DSL 约束验证
        logical_result = []
        logical_pass = True
        print("DSL constraint validation disabled - treating all datasets equally.")

        # # 以下代码已禁用 - 不再读取或使用 hard_logic_py
