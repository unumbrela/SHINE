...
class AbstractLLM(ABC):
    ...
    def _check_and_raise_balance_error(self, exception):
        """
        Unified method to check if an exception is related to insufficient API balance.
        ...
        Raises:
            InsufficientBalanceError: If the exception is balance-related

        Returns:
            None: If the exception is not balance-related (caller should handle it)

        Usage in subclasses:
            try:
                response = self.llm.chat.completions.create(...)
            except Exception as e:
                self._check_and_raise_balance_error(e)  # Will raise if balance error
                # Handle other errors here
                res_str = '{"error": "Request failed"}'
        """
        error_msg = str(exception).lower()

        # Comprehensive list of balance-related keywords across different API providers
        balance_keywords = [
            # Chinese keywords
            '余额不足',
            '账户余额不足',
        ...
        # Check if any balance keyword appears in the error message
...
class GLM46(AbstractLLM):
    ...
    def _get_response(self, messages, one_line, json_mode):
        ...
        try:
            ...
            print(f"[MyAgent GLM46 Error] {type(e).__name__}: {e}")
            res_str = '{"error": "Request failed."}'
...
class Deepseek(AbstractLLM):
    ...
    def _get_response(self, messages, one_line, json_mode):
        ...
        try:
            ...
            self._check_and_raise_balance_error(e)
            # If not a balance error, handle as normal error
            print(f"[MyAgent Deepseek Error] {type(e).__name__}: {e}")
...
class GLM4Plus(AbstractLLM):
    ...
    def _get_response(self, messages, one_line, json_mode):
        ...
        try:
            ...
            self._check_and_raise_balance_error(e)
            # If not a balance error, handle as normal error
            print(f"[MyAgent GLM4Plus Error] {type(e).__name__}: {e}")
            res_str = '{"error": "Request failed, please try again."}'
        return res_str
...
class GPT4o(AbstractLLM):
    ...
    def _get_response(self, messages, one_line, json_mode):
        ...
        try:
            ...
            self._check_and_raise_balance_error(e)
            # If not a balance error, handle as normal error
            print(f"[MyAgent GPT4o Error] {type(e).__name__}: {e}")
            res_str = '{"error": "Request failed, please try again."}'
        return res_str
