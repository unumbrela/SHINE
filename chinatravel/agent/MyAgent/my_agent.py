...
try:
    ...
    from .my_base import AbstractAgent, BaseAgent
except ImportError:
    # 如果没有，使用父目录的
...
# Import InsufficientBalanceError from main project's llms
# This must match the exception type raised by backbone_llm (which uses llms.py, not my_llms.py)
...
class TPCAgent(BaseAgent):
    ...
    def run(self, query, prob_idx, oralce_translation=True):
        ...
        # Add token and statistics info to output (same format as LLMNeSy/UrbanTrip)
        ...
        if hasattr(self, 'llm_rec_format_error'):
...
不要包含其他解释文字，只返回数字序列。"""
            ...
            # Re-raise balance errors to trigger interruption
            raise
