...
class AbstractLLM(ABC):
    ...
    def __init__(self):
        ...
        self.llm_call_time = 0.0  # Total time spent in LLM calls (seconds)
        pass
    ...
    def _check_and_raise_balance_error(self, exception):
        ...
        Args:
            exception: The caught exception to check

        Raises:
            InsufficientBalanceError: If the exception is balance-related or authentication-related

        Returns:
            None: If the exception is not balance/auth-related (caller should handle it)

        Usage in subclasses:
            try:
                response = self.llm.chat.completions.create(...)
            except Exception as e:
                self._check_and_raise_balance_error(e)  # Will raise if balance/auth error
...
class Deepseek(AbstractLLM):
    ...
    def _send_request(self, messages, kwargs):
        ...
        end_time = time.time()
        self.llm_call_time += (end_time - start_time)
...
class GLM4Plus(AbstractLLM):
    ...
    def _send_request(self, messages, kwargs):
        ...
        try:
            ...
            if any(keyword in error_str for keyword in ['rate limit', 'too many requests', '429', 'quota', 'limit exceeded']):
                print(f"[GLM-4-Plus RATE LIMIT] {e}")
            raise e

        end_time = time.time()
        elapsed = end_time - start_time
        self.llm_call_time += elapsed
    ...
    def _get_response(self, messages, one_line, json_mode):
        ...
        try:
            ...
            self._check_and_raise_balance_error(e)
            # If not a balance error, handle as normal error
            print(f"[GLM4Plus Error] {type(e).__name__}: {e}")
            res_str = '{"error": "Request failed, please try again."}'
        return res_str
...
class GLM46(AbstractLLM):
    def __init__(self):
        ...
        try:
            ...
            import os
            # Get API key from OPENAI_API_KEY (for compatibility) or ZAI_API_KEY
            api_key = os.environ.get('OPENAI_API_KEY') or os.environ.get('ZAI_API_KEY')
            self.llm = ZhipuAiClient(api_key=api_key)
            self.use_new_sdk = True
            print(f"[GLM-4.6] Using new zai-sdk (max_tokens=65536)")
        except ImportError:
            ...
            print(f"[GLM-4.6] Using OpenAI-compatible API (max_tokens=4095). Install zai-sdk for better support: pip install zai-sdk")
        self.name = "GLM46"
    ...
    def _get_response(self, messages, one_line, json_mode):
        kwargs = {
            "model": "glm-4.6",
            "max_tokens": 65536 if self.use_new_sdk else 4095,  # New SDK supports more tokens
            "temperature": 0,
            "top_p": 0.01,
        }
        # Enable thinking mode for better reasoning (only for new SDK)
        if self.use_new_sdk:
        ...
        try:
            ...
            # Check for balance-related errors first (using unified method)
            self._check_and_raise_balance_error(e)
            # If not a balance error, handle as normal error
            print(f"[GLM46 Error] {type(e).__name__}: {e}")
            res_str = '{"error": "Request failed, please try again."}'
        return res_str
...
class Qwen(AbstractLLM):
    def __init__(self, model_name, max_model_len=None):
        super().__init__()
        self.path = os.path.join(
            project_root_path, "..", "local_llm", model_name
        )
    ...
    def _get_response(self, messages, one_line, json_mode):
        ...
        if "Qwen3" in self.name:
            text = self.tokenizer.apply_chat_template(
                ...
                enable_thinking=False # Switch between thinking and non-thinking modes. Set to False for faster response.
            )
...
class Mistral(AbstractLLM):
    def __init__(self, max_model_len=None):
        super().__init__()
        self.path = os.path.join(
            project_root_path, "..", "local_llm", "Mistral-7B-Instruct-v0.3",
        )
...
class Llama(AbstractLLM):
    def __init__(self, model_name, max_model_len: int | None = 8192):
        super().__init__()

        Llama_supported = ["Llama3-3B", "Llama3-8B"]
        if model_name not in Llama_supported:
            raise ValueError(
                f"Unsupported model name: {model_name}. Supported models: {Llama_supported}"
            )

        if model_name == "Llama3-3B":
            self.path = os.path.join(
                project_root_path, "..", "local_llm", "Llama-3.2-3B-Instruct"
            )
        elif model_name == "Llama3-8B":
            self.path = os.path.join(
                project_root_path, "..", "local_llm", "Meta-Llama-3.1-8B-Instruct"
            )
        ...
        self.llm = LLM(model=self.path, max_model_len=self.max_model_len)
        self.name = model_name
    ...
    def _get_response(self, messages, one_line, json_mode):
        ...
        if len(input_tokens) >= self.max_model_len:
            return (
                '{"error": '
                f'"Input prompt is longer than {self.max_model_len} tokens."'
                "}"
            )
